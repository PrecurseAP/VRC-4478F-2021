...
int purePursuit(std::vector<Point> path, float lookahead, robotPosition* robotPose) {
  ...
  while(!done) {
    ...
    mBackLeft.spin(forward, targetLeftPower, rpm);
    mFrontRight.spin(forward, targetRightPower, rpm);
    mMidRight.spin(forward, targetRightPower, rpm);
    mBackRight.spin(forward, targetRightPower, rpm);
